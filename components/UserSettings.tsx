...
import { Trash2, ExternalLink, Youtube, Upload, CheckCircle2, AlertCircle, BarChart2, Search } from 'lucide-react';
  ...
  const handleBulkImport = async () => {
    ...
    lines.forEach(line => {
      ...
      if (line.includes('/channel/')) {
          ...
          invalidLines.push(line); // Handles not supported yet
          return;
      }
    ...
    if (newChannels.length > 0) {
      ...
      if (success) {
          ...
          showNotification('Error saving channels. IDs might differ.', 'error');
      }
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-slate-500 mt-1">Bind and manage your YouTube distribution channels.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
             <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm h-full flex flex-col">
                ...
                <textarea 
                    ...
                    placeholder={"Paste YouTube Links or Channel IDs here...\nOne per line."}
                />
                <button onClick={handleBulkImport} disabled={isImporting || !bulkInput} className="w-full mt-4 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-all disabled:opacity-50 shadow-lg shadow-indigo-200">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[600px]">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h3 className="font-bold text-slate-800">Active Bindings</h3>
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none w-full sm:w-64"
                    />
                </div>
            </div>
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredChannels.length === 0 ? (
                            <tr><td colSpan={3} className="px-6 py-12 text-center text-slate-400">No channels found.</td></tr>
                            ...
                            <tr key={ch.id} className="hover:bg-slate-50 transition-colors group">
                                <td className="px-6 py-4 font-mono text-slate-600 flex items-center gap-3">
                                    ...
                                    <a href={`https://youtube.com/channel/${ch.channelId}`} target="_blank" className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-indigo-600 transition-opacity"><ExternalLink className="w-3 h-3" /></a>
                                </td>
                                <td className="px-6 py-4 text-right">
                                    <button onClick={() => onViewChannelStats(ch.channelId)} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:border-indigo-300 hover:text-indigo-600 transition-colors inline-flex items-center gap-1"><BarChart2 className="w-3 h-3" /> Stats</button>
                                </td>
                                <td className="px-6 py-4 text-right">
