...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout, activeTab, onTabChange }) => {
  ...
  const menuItems = isAdmin 
    ...
    : [
        ...
        { 
            id: 'settings', 
            label: 'Channel Manager', // Renamed for clarity
            icon: hasChannelBound ? Radio : AlertTriangle, // Changed icon to Radio/Broadcast
            highlight: !hasChannelBound 
        },
      ];

  return (
    <div className="min-h-screen bg-slate-950 flex text-slate-200 font-inter selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed lg:static inset-y-0 left-0 z-30 w-64 bg-slate-900/60 backdrop-blur-xl border-r border-white/5 transform transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        <div className="p-4">
          <div className="flex items-center space-x-3 mb-8 p-4 bg-white/5 rounded-2xl border border-white/5">
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-500 flex items-center justify-center font-bold text-white shadow-inner text-sm">
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isActive 
                          ? 'bg-gradient-to-r from-indigo-600/90 to-violet-600/90 text-white shadow-md shadow-indigo-900/20' 
                          : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'}
                    `}
                  >
                    <Icon className={`w-5 h-5 transition-colors ${isActive ? 'text-white' : isHighlight ? 'text-amber-400 animate-pulse' : 'text-slate-500 group-hover:text-slate-300'}`} />
                    <span className={`font-medium text-sm ${isHighlight ? 'text-amber-400' : ''}`}>{item.label}</span>
        ...
        <div className="absolute bottom-0 w-full p-4 border-t border-white/5">
          <button
            onClick={onLogout}
            className="w-full flex items-center space-x-3 px-3 py-3 text-slate-400 hover:bg-red-500/10 hover:text-red-400 rounded-xl transition-all"
          >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-20">
            ...
            <div className="relative z-10 flex items-center">
                <button 
                    className="lg:hidden p-2 -ml-2 text-slate-400 mr-2"
            ...
            <div className="relative z-10 flex items-center gap-4">
                <div className="hidden md:flex items-center px-3 py-1.5 bg-slate-900/50 rounded-full border border-white/10">
