...
import { User, UserRole, CsvRow, ChannelStats, MusicTrack, VideoStat, ChannelCatalogItem } from './types';
import { dataService } from './services/dataService';
import { ArrowLeft, Sparkles, Loader2, CheckCircle2, XCircle, Info, Copy, Check } from 'lucide-react';
  ...
  const handleDataParsed = async (rows: CsvRow[]) => {
    ...
    rows.forEach(row => {
        ...
        if (ownerInfo) {
            ...
            // --- User Aggregate ---
            if (!userAggregates[userId]) userAggregates[userId] = { views: 0, premium: 0, gross: 0, net: 0, daily: {}, countries: {}, videos: {}, channels: {} };
            ...
            // Channel Catalog
            if (!uAgg.channels[row.channelId]) uAgg.channels[row.channelId] = { views: 0, premium: 0, net: 0 };
            uAgg.channels[row.channelId].views += views;
            uAgg.channels[row.channelId].premium += premium;
            uAgg.channels[row.channelId].net += netCalculated;

            // --- Channel Detail Aggregate ---
    ...
    for (const [cid, raw] of Object.entries(channelAggregates) as any[]) {
        ...
        await dataService.saveChannelStats(cid, { totalViews: raw.views, totalPremiumViews: raw.premium, totalRevenue: raw.net, dailyStats, topCountries, topVideos, channelCatalog: [], lastUpdated: new Date().toISOString() });
    }
    ...
    for (const [uid, raw] of Object.entries(userAggregates) as any[]) {
        ...
        // Build Catalog
        const channelCatalog: ChannelCatalogItem[] = Object.entries(raw.channels).map(([cid, d]: any) => ({
            channelId: cid, views: d.views, premiumViews: d.premium, revenue: d.net
        })).sort((a, b) => b.revenue - a.revenue);

        const statObj: ChannelStats = {
            totalViews: raw.views, totalPremiumViews: raw.premium, totalRevenue: raw.net,
            dailyStats, topCountries, topVideos, channelCatalog, lastUpdated: new Date().toISOString()
        };
