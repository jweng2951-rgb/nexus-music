...
function App() {
  ...
  const handleDataParsed = async (rows: CsvRow[]) => {
    ...
    for (const [cid, raw] of Object.entries(channelAggregates) as any[]) {
        const dailyStats = Object.entries(raw.daily).map(([date, d]: any) => ({ date, views: d.views, premiumViews: d.premium, estimatedRevenue: d.net, grossRevenue: 0 })).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        const topCountries = Object.entries(raw.countries).map(([code, d]: any) => ({ code, views: d.views, revenue: d.net })).sort((a, b) => b.views - a.views);
        await dataService.saveChannelStats(cid, { totalViews: raw.views, totalPremiumViews: raw.premium, totalRevenue: raw.net, dailyStats, topCountries, lastUpdated: new Date().toISOString() });
    }
    ...
    for (const [uid, raw] of Object.entries(userAggregates) as any[]) {
        const dailyStats = Object.entries(raw.daily).map(([date, d]: any) => ({ date, views: d.views, premiumViews: d.premium, estimatedRevenue: d.net, grossRevenue: 0 })).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        const topCountries = Object.entries(raw.countries).map(([code, d]: any) => ({ code, views: d.views, revenue: d.net })).sort((a, b) => b.views - a.views);
        const statObj = { totalViews: raw.views, totalPremiumViews: raw.premium, totalRevenue: raw.net, dailyStats, topCountries, lastUpdated: new Date().toISOString() };
        await dataService.saveUserStats(uid, statObj);
        newUserStats[uid] = statObj;
    }
    
    setUserStatsMap(newUserStats);
    showToast(`Sync Complete! Matched ${matchedRows} rows.`, 'success');
  };

  const handleAddUser = async (user: Partial<User>) => {
    setIsLoading(true);
    await dataService.saveUser({ id: user.id || `user-${Date.now()}`, username: user.username!, password: user.password!, role: user.role || UserRole.USER, revenueShare: user.revenueShare || 70, createdAt: new Date().toISOString(), status: 'active' });
  ...
  const handleUpdateUser = async (id: string, updates: Partial<User>) => {
    ...
    if (user) { await dataService.saveUser({ ...user, ...updates }); setAllUsers(await dataService.getUsers()); showToast('User updated'); }
    setIsLoading(false);
  };

  const handleDeleteUser = async (id: string) => {
    if (window.confirm('Delete user?')) { setIsLoading(true); await dataService.deleteUser(id); setAllUsers(await dataService.getUsers()); setIsLoading(false); showToast('User deleted'); }
  };

  const handleExport = () => {
    const content = "data:text/csv;charset=utf-8," + ['ID,Username,Role,Share,Status'].join(",") + "\n" + allUsers.map(u => [u.id, u.username, u.role, u.revenueShare + '%', u.status].join(",")).join("\n");
        ...
        <div className="min-h-screen w-full flex items-center justify-center p-4 bg-slate-50">
             <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    <div><label className="text-xs font-bold text-slate-500 uppercase">Access ID</label><input type="text" className="w-full mt-1 p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none" value={loginUsername} onChange={e=>setLoginUsername(e.target.value)} /></div>
                    <div><label className="text-xs font-bold text-slate-500 uppercase">Secure Key</label><input type="password" className="w-full mt-1 p-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-indigo-500 outline-none" value={loginPassword} onChange={e=>setLoginPassword(e.target.value)} /></div>
                    <button type="submit" className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-indigo-100">Login</button>
                </form>
                {loginError && <div className="mt-4 p-3 bg-red-50 text-red-600 rounded-xl text-sm text-center font-medium">{loginError}</div>}
                <div className="mt-6 text-center"><button onClick={()=>{navigator.clipboard.writeText(window.location.href);setCopyLink(true);setTimeout(()=>setCopyLink(false),2000)}} className="text-xs text-slate-400 hover:text-indigo-600 flex items-center justify-center gap-1 mx-auto">{copyLink?<Check className="w-3 h-3"/>:<Copy className="w-3 h-3"/>} Copy Portal Link</button></div>
             </div>
        </div>
      );
  }
  ...
  return (
    <Layout user={currentUser} onLogout={handleLogout} activeTab={activeTab} onTabChange={(tab) => { setActiveTab(tab); if(tab!=='dashboard') setSelectedUserId(null); }}>
        <div className="fixed top-4 right-4 z-50 flex flex-col gap-3 pointer-events-none">
            {toasts.map(t => (
                <div key={t.id} className={`toast-enter pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg border bg-white ${t.type==='success'?'border-emerald-100 text-emerald-700':t.type==='error'?'border-red-100 text-red-700':'border-blue-100 text-blue-700'}`}>
                    {t.type==='success'?<CheckCircle2 className="w-5 h-5"/>:t.type==='error'?<XCircle className="w-5 h-5"/>:<Info className="w-5 h-5"/>}<span className="font-medium">{t.message}</span>
                </div>
            ))}
        </div>
        {activeTab === 'channel_detail' && (
            <div className="space-y-6 animate-fade-in">
                <button onClick={handleBackFromChannel} className="flex items-center space-x-2 text-slate-500 hover:text-indigo-600 transition-colors group"><div className="p-2 rounded-full bg-white border border-slate-200 group-hover:border-indigo-200 transition-colors"><ArrowLeft className="w-4 h-4" /></div><span className="font-bold">Back to List</span></button>
                <DashboardStats title={`Channel: ${viewingChannelId}`} stats={viewingChannelStats} revenueShare={currentUser.role===UserRole.ADMIN?100:currentUser.revenueShare} showShareInfo={false} />
            </div>
        )}
